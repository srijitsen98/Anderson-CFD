...
int subsonicas(double pe)
{
	double Me,temp;
	double c1,c2;
	c1=gamma+1;
	c2=gamma-1;
	Me=2.0*(pow(pe,-(gamma-1)/gamma)-1.0)/(gamma-1);
	Me=sqrt(Me);
	printf("\nMe is %f",Me);
	temp=(1/(Me*Me))*pow((2/c1)*(1+0.5*c2*Me*Me),c1/c2);
	temp=sqrt(temp);
	As=A[nx-1]/temp;
	return 0;
}
...
int analytical()
{
    ...
    for(i=0;i<nx;i++)
    {
		...
		printf("\nM %f p %f rho %f T %f V %f A %f s %d",Man[i],pan[i],rhoan[i],Tan[i],Van[i],A[i],s[i]);
    }
	//getch();
	return 0;
}
int varinit()
{
    ...
    for(i=0;i<nx;i++)
    {
        ...
        rho[i]=1.0-0.023*i*dx;
		T[i]=1.0-0.009333*i*dx;
		V[i]=0.05+0.11*i*dx;
		M[i]=V[i]/sqrt(T[i]);
        p[i]=rho[i]*T[i];
...
int main()
{
    ...
    //Area initialization
    areainit(); //checked
	//Subsonic As (Subsonic case only)
	subsonicas(pe);
    printf("\nAs is %f",As);
	//Analytical Solution
    analytical(); //checked
