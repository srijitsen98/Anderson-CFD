...
int analytical()
{
    ...
    for(i=0;i<nx;i++)
    {
		...
		{
			Man[i]=bisection(A[i]/As2,s[i]);
			pan[i]=0.6682*pow((1+(gamma-1)*0.5*Man[i]*Man[i]),-gamma/(gamma-1));
			Tan[i]=pow((1+(gamma-1)*0.5*Man[i]*Man[i]),-1.0);
			rhoan[i]=0.6882*pow((1+(gamma-1)*0.5*Man[i]*Man[i]),-1.0/(gamma-1));
		}
...
int varinit()
{
    ...
    for(i=0;i<nx;i++)
    {
		...
		{
			rho[i]=0.634-0.702*(i*dx-1.5);
			T[i]=0.833-0.4908*(i*dx-1.5);
		}
		else if(i*dx<=3.01)
		{
			rho[i]=0.5892-0.10228*(i*dx-2.1);
			T[i]=0.93968-0.0622*(i*dx-2.1);
		}
		V[i]=0.59/(rho[i]*A[i]);
...
int mac()
{
    ...
    {
        U1b[i]=U1[i]+dU1[i]*dt;
		S1b[i]=Cx*(U1[i+1]-2*U1[i]+U1[i-1])*fabs(p[i+1]-2*p[i]+p[i-1])/(p[i+1]+2*p[i]+p[i-1]);
		U1b[i]+=S1b[i];
		U2b[i]=U2[i]+dU2[i]*dt;
		S2b[i]=Cx*(U2[i+1]-2*U2[i]+U2[i-1])*fabs(p[i+1]-2*p[i]+p[i-1])/(p[i+1]+2*p[i]+p[i-1]);
		U2b[i]+=S2b[i];
		U3b[i]=U3[i]+dU3[i]*dt;
		S3b[i]=Cx*(U3[i+1]-2*U3[i]+U3[i-1])*fabs(p[i+1]-2*p[i]+p[i-1])/(p[i+1]+2*p[i]+p[i-1]);
		U3b[i]+=S3b[i];
		F1b[i]=U2b[i];
    ...
    U2b[0]=2*U2b[1]-U2b[2];
    Vb[0]=U2b[0]/U1b[0];
	U3b[0]=U1b[0]*(T[0]/(gamma-1)+(gamma/2.0)*Vb[0]*Vb[0]);
    i=0;
	F1b[i]=U2b[i];
	F2b[i]=(U2b[i]*U2b[i])/U1b[i]+((gamma-1)/gamma)*(U3b[i]-(gamma*0.5)*U2b[i]*U2b[i]/U1b[i]);
	F3b[i]=gamma*U3b[i]*U2b[i]/U1b[i]-gamma*(gamma-1)*0.5*pow(U2b[i],3.0)/pow(U1b[i],2.0);
	...
	//Right Boundary
	U1b[nx-1]=2.0*U1b[nx-2]-U1b[nx-3];
    U2b[nx-1]=2.0*U2b[nx-2]-U2b[nx-3];
    //U3[nx-1]=2.0*U3[nx-2]-U3[nx-3];
	Vb[nx-1]=U2b[nx-1]/U1b[nx-1];
	U3b[nx-1]=pe*A[i]/(gamma-1)+(gamma/2.0)*Vb[nx-1]*U2b[nx-1];
	i=nx-1;
	rhob[i]=U1b[i]/A[i];
	Vb[i]=U2b[i]/U1b[i];
	Tb[i]=(gamma-1)*((U3b[i]/U1b[i])-(gamma/2.0)*(U2b[i]/U1b[i])*(U2b[i]/U1b[i]));
	pb[nx-1]=rhob[nx-1]*Tb[nx-1];
	*/
	//Corrector Step
    for(i=1;i<nx-1;i++)
    {
    ...
    for(i=1;i<nx-1;i++)
    {
		...
		S1[i]=Cx*(U1b[i+1]-2*U1b[i]+U1b[i-1])*fabs(pb[i+1]-2*pb[i]+pb[i-1])/(pb[i+1]+2*pb[i]+pb[i-1]);
		U1[i]+=S1[i];
		U2[i]=U2[i]+(dU2b[i]+dU2[i])*0.5*dt;
		S2[i]=Cx*(U2b[i+1]-2*U2b[i]+U2b[i-1])*fabs(pb[i+1]-2*pb[i]+pb[i-1])/(pb[i+1]+2*pb[i]+pb[i-1]);
		U2[i]+=S2[i];
		U3[i]=U3[i]+(dU3b[i]+dU3[i])*0.5*dt;
		S3[i]=Cx*(U3b[i+1]-2*U3b[i]+U3b[i-1])*fabs(pb[i+1]-2*pb[i]+pb[i-1])/(pb[i+1]+2*pb[i]+pb[i-1]);
		U3[i]+=S3[i];
...
int boundary()
{
    ...
    //U3[nx-1]=2.0*U3[nx-2]-U3[nx-3];
	V[nx-1]=U2[nx-1]/U1[nx-1];
	U3[nx-1]=pe*A[i]/(gamma-1)+(gamma/2.0)*V[nx-1]*U2[nx-1];
	i=nx-1;
	F1[i]=U2[i];
	F2[i]=(U2[i]*U2[i])/U1[i]+((gamma-1)/gamma)*(U3[i]-(gamma*0.5)*U2[i]*U2[i]/U1[i]);
	F3[i]=gamma*U3[i]*U2[i]/U1[i]-gamma*(gamma-1)*0.5*pow(U2[i],3.0)/pow(U1[i],2.0);
...
int main()
{
	...
	for(i=0;i<nx;i++)
		printf("\nI %d M %f E %f p %f E %f rho %f E %f T %f E %f V %f E %f A %f",i+1,M[i],Mean[i],p[i],pean[i],rho[i],rhoean[i],T[i],Tean[i],V[i],Vean[i],A[i]);
