...
int main()
{
    ...
    for(j=0;j<ydiv;j++)
    {
        ...
        printf("\n%d %d %f %f %f %f %f %f %f",i,j,fg.u[i][j],fg.v[i][j],fg.rho[i][j],fg.p[i][j],fg.T[i][j],fg.M[i][j],fg.F[i][j][4]);
    ...
    while(i<16)
    {
        ...
        printf("\nFdiffcorr0 %f %f %f %f",dFdxipred[0][1],dFdxipred[0][2],dFdxipred[0][3],dFdxipred[0][4]);
        for(j=0;j<ydiv-1;j++)
        {
            for(k=1;k<=4;k++)
            {
